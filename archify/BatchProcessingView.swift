...
struct BatchProcessingView: View {
    ...
    @State private var sortOrder: SortOrder = .name(ascending: true)
    ...
    var resultsSection: some View {
        VStack(alignment: .center, spacing: 20) {
            ...
            PieChartView(
                savedSpaces: batchProcessing.savedSpaces,
                appSizes: batchProcessing.appSizes.filter { batchProcessing.selectedApps.contains($0.0) }
            )
            .frame(height: 300)
            .padding()
        }
    ...
    var filteredAndSortedApps: [(String, UInt64, UInt64)] {
        ...
        case .size(let ascending):
            return filtered.sorted { ascending ? $0.1 < $1.1 : $0.1 > $1.1 }
        }
    }
}

struct PieChartView: View {
    let savedSpaces: [String: UInt64]
